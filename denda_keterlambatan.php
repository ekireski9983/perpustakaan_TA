                    ...
                    <tbody>
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                echo '<td class="status-cell">' . htmlspecialchars($row['status_pembayaran']) . '</td>'; // Tambahkan class untuk akses mudah
                                echo '<td>
                                        <button class="btn btn-sm btn-edit" data-bs-toggle="modal" data-bs-target="#ubahStatusdendaModal" 
...
<div class="modal fade" id="ubahStatusdendaModal" tabindex="-1" aria-labelledby="ubahStatusdendaModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="ubahStatusdendaModalLabel">Ubah Status Pengembalian</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p>silahkan anda bayar secara offline</p>
                <div class="d-flex justify-content-between mt-3">
                    <button type="button" class="btn btn-update-status-belum" data-status-value="sudah dibayarkan">bayar denda</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    // Listener untuk tombol "bayarkan denda" di dalam modal
    document.querySelectorAll('#ubahStatusdendaModal .btn').forEach(button => {
        button.addEventListener('click', function() {
            if (currentBookIdToUpdate) {
                const newStatus = this.getAttribute('data-status-value');

                // Kirim permintaan fetch ke PHP
                fetch('denda_kelola.php', {
                    method: 'POST',
                    headers: {
